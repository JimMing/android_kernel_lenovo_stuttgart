...
unsigned char mc_asoc_audio_mode_infm(void);
...
dbg_info("switch_set_state 0\n");
	...
	if(dFlags & MCDRV_HSDET_EVT_DLYKEYON0_FLAG) {
		...
		if((bMicDet != 0) && !(dFlags & MCDRV_HSDET_EVT_KEYOFF0_FLAG) && !!(dFlags & MCDRV_HSDET_EVT_KEYON0_FLAG)) {
	...
	if(!!(dFlags & MCDRV_HSDET_EVT_DLYKEYOFF0_FLAG) && 
       !!(dFlags & MCDRV_HSDET_EVT_KEYON0_FLAG) && !!(dFlags & MCDRV_HSDET_EVT_KEYOFF0_FLAG)) {
		if(mc_asoc_hwid_a == MCDRV_HWID_15H) {
...
unsigned char mc_asoc_audio_mode_infm(void)
{
    ...
    printk("audio_mode = %#x", mc_asoc->audio_mode);
    if(mc_asoc->audio_mode >> 4){
        printk("FM On");
        return 1;
    } else {
        printk("FM Off");
	...
	pm_message_t state)
{
	...
	printk("[%s][%d]\n",__func__, __LINE__);
	mc_asoc	= mc_asoc_get_mc_asoc(codec);
	if(mc_asoc_audio_mode_incall() || mc_asoc_audio_mode_infm()){
