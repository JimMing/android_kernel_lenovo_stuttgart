...
static int bcmdhd_wifi_power_state;
...
*/
	...
	mdelay(200);
	bcmdhd_wifi_power_state = on;
	return 0;
}

static int bcmdhd_wifi_reset_state;
int bcmdhd_wifi_reset(int on)
{
	printk("%s: do nothing\n", __func__);
	bcmdhd_wifi_reset_state = on;
	return 0;
}

static void *bcmdhd_wifi_mem_prealloc(int section, unsigned long size);
int bcmdhd_wifi_status_register(
...
#else
	set->ext_cd_init = bcmdhd_wifi_status_register;
#endif
...
static int __init bcmdhd_wifi_init(void)
{
	...
	bcmdhd_wifi_device.resource->start = gpio_to_irq(EXYNOS4_GPX2(7));
	bcmdhd_wifi_device.resource->end = gpio_to_irq(EXYNOS4_GPX2(7));
	ret = platform_device_register(&bcmdhd_wifi_device);
        return ret;
}

module_init(bcmdhd_wifi_init);
#endif
