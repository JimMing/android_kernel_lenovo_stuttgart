...
static int g_lcd_nova = 2;	////0:none nava chip  1: nova chip  2:no lcd_nova para in command line
static int __init lcd_nava_check(char *p)
{
	g_lcd_nova = simple_strtoul(p, NULL, 0);;
	printk("####get g_lcd_nova=%d#####\n", g_lcd_nova);
	return 0;
}
early_param("lcd_nova", lcd_nava_check);
...
static void gpio_cfg(void)
{
	...
	mdelay(5);
	gpio_val = gpio_get_value(ID_PIN);
	if(g_lcd_nova == 1)//
	{
		if(gpio_val)
			...
			lcd_id = LCD_NOVA_CMI;
		printk("********************** lcd_id = %s ***************************\n", gpio_val ? "LCD_NOVA_YASSY" : "LCD_NOVA_CMI");
	}else	//0 or 2
	{
		if(gpio_val)
			lcd_id = LCD_LG;
...
static void __init mipi_fb_init(void)
{
	...
	if (lcd_id == LCD_BOE) {
		...
		dsim_lcd_info->lcd_panel_info = (void *)&stuttgart_mipi_lcd_yassy;
	else//yassy
		dsim_lcd_info->lcd_panel_info = (void *)&stuttgart_mipi_lcd_yassy;
...
static struct max77686_platform_data exynos4_max77686_info = {
	...
	.buck3_voltage[0] = 1100000,	/* 1.0375V */
