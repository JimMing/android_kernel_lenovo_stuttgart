...
static ssize_t bluetooth_wakelock_write(struct file *file, const char __user *buf, ssize_t nbytes, loff_t *ppos)
{
    ...
    if(nbytes > 2)
     {
       printk("error nbytes=%d are longer then two character return",nbytes);
       return nbytes;
     }


  //  sscanf(buf, "%s", s`tring);
    strncpy(string,buf,1);
    if (!strcmp((const char *)string, (const char *)"0")){
        bt_wake = 0;
        printk("brcm_wake_unlock \n");
        wake_unlock(&brcm_lock); 
     //   wake_lock_timeout(&brcm_lock,10*HZ);
    }
     else if (!strcmp((const char *)string, (const char *)"1")) {
            bt_wake = 1;
            printk("brcm_wakelock \n");
          wake_lock(&brcm_lock);
     } else {
         printk("enter bluetooth_wakelock_write \n");
     }
     return nbytes;
}
