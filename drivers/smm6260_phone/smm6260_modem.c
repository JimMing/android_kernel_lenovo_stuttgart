...
static int modem_suspend(struct device *dev)
{
	...
	/* Gao Liankai: Disable the cp_reset_out irq, so it won't disturb 
                        us before calling modem_resume during system resume */
...
static int modem_resume(struct device *dev)
{
	...
	/* Gao Liankai: Enable the cp_reset_out irq which was 
                        disabled in modem_suspend */
	enable_irq(mc->irq[0]);
