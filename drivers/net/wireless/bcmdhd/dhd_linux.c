...
static int dhd_add_remove_eventmsg(dhd_pub_t *dhd, u16 event, bool add)
{
	...
	if (unlikely(err)) {
		DHD_ERROR(("%s: Get event_msgs error (%d)\n", __FUNCTION__, err));
		goto eventmsg_out;
	}
	...
	err = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
	if (unlikely(err)) {
		DHD_ERROR(("%s: Set event_msgs error (%d)\n", __FUNCTION__, err));
		goto eventmsg_out;
	}
	...
	if (dhd && dhd->up) {
		if (value && dhd->in_suspend) {
				...
				if ((dhd->op_mode & WFD_MASK) == WFD_MASK &&
				     dhd_check_ap_wfd_mode_set(dhd) == FALSE) {
					printk(KERN_ERR "[%s] remove P2P_PROBREQ mask.\n",  __FUNCTION__);
					dhd_add_remove_eventmsg(dhd, WLC_E_P2P_PROBREQ_MSG, 0);
				}
			} else {
				...
				if ((dhd->op_mode & WFD_MASK) == WFD_MASK) {
					printk(KERN_ERR "[%s] add P2P_PROBREQ mask.\n",  __FUNCTION__);
					dhd_add_remove_eventmsg(dhd, WLC_E_P2P_PROBREQ_MSG, 1);
				}
			}
	}
...
extern struct net_device* dhd_p2p_net;
/* Flags to reset the P2P while system hang. */
static int dhd_reset_p2p = 0;
...
dhd_open(struct net_device *net)
{
	...
	if(dhd_reset_p2p) {
		printk(KERN_ERR "DHD: Reset the P2P interface after hang.\n");
		dhd_reset_p2p = 0;
		if(dhd_p2p_net != NULL) {
			dev_open(dhd_p2p_net);
		}
	}
exit:
	if (ret)
...
int net_os_send_hang_message(struct net_device *dev)
{
	...
	printk(KERN_ERR "DHD: Set the P2P interface reset flag.\n");
	dhd_reset_p2p = 1;
	if (dhd) {
		if (!dhd->pub.hang_was_sent) {
...
EXPORT_SYMBOL(net_os_send_hang_message);
#endif 
