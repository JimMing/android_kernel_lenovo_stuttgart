...
void print_fimd_regs(void)
{
	unsigned int i,j;

	printk("##print_fimd_regs###########\r\n");
	if(fbfimd)
	{
		for(i=0; i<=fbfimd->dual; i++)
		{
			if(fbfimd->fbdev[i]->regs != NULL)
			{
				for(j=0; j<=0x3c; j=j+4)
					printk("0x%8x @ 0x%x  \r\n",readl(fbfimd->fbdev[i]->regs + j), j);
			}
		}
	}
	printk("==print_fimd_regs===========\r\n");
...
static int s3cfb_probe(struct platform_device *pdev)
{
	...
	int i = 0;
#ifdef CONFIG_REGULATOR
	lcd_regulator = regulator_get(NULL, "vdd_ldo20");
	...
	for (i = 0; i < FIMD_MAX; i++) {
		...
		//fbdev[i]->early_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN - 30;
...
void s3cfb_early_suspend(struct early_suspend *h)
{
	...
	for (i = 0; i < FIMD_MAX; i++) {
		...
		for (j = 0; j < pdata->nr_wins; j++) {
			...
			{
				//printk("#####################\n");
				memset(fb->screen_base, 0 , fb->fix.smem_len);	//fix the bug that lcd show the last frame before suspend when resume the device
			}
		}
#endif
	}
