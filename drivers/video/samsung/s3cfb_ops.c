...
int s3cfb_draw_logo(struct fb_info *fb)
{
	...
	int w, h;
	u32 color;
	u32 *pBuffer = (u32*)fb->screen_base;

	w = fb->var.xres;
	h = fb->var.yres;
	pBuffer +=  w * ROCKET_LOGO_START_Y +  ROCKET_LOGO_START_X;
	for (i = 0; i < ROCKET_LOGO_HEIGHT; i++)
	{
		for (j = 0; j < ROCKET_LOGO_WIDTH; j++)
		{
			color = rocket_logo_data[i * ROCKET_LOGO_WIDTH + j];
			color = (color << 16) | (color << 8) | color;
			*pBuffer++ = color;
		}
		pBuffer += w - ROCKET_LOGO_WIDTH;
	}
...
int s3cfb_unmap_video_memory(struct s3cfb_global *fbdev, struct fb_info *fb)
{
	...
	//When use direct map fbuf, need flush cache after write to fb by SW. Jiangshanbin 20120713
	dmac_flush_range(phys_to_virt(fb->fix.smem_start),
				 phys_to_virt(fb->fix.smem_start +fb->fix.smem_len));
	outer_flush_range(fb->fix.smem_start,
				 (fb->fix.smem_start +fb->fix.smem_len));	/* L2 */
	return 0;
}
...
int s3cfb_map_video_memory(struct s3cfb_global *fbdev, struct fb_info *fb)
{
	...
	fb->screen_base = cma_get_virt(fix->smem_start, PAGE_ALIGN(fix->smem_len), 0);
#else
